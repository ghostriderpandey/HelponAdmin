...
namespace HelponAdminNew.AP
{
    public partial class Master_Bank : System.Web.UI.Page
    {
        ...
        {
            DataTable dtData = cls.selectDataTable("ProcMaster_Bank 'GetAll'");
            GvData.DataSource = dtData;
            GvData.DataBind();
        }
        private void GetData(int id)
        {
            DataTable dtresult = cls.selectDataTable("ProcMaster_Bank 'GetbyID','" + id + "'");
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt = cls.selectDataTable("Exec ProcMaster_Bank 'insert','" + id + "','" + txtName.Text.Replace("'", "").Trim() + "'");
            if (dt.Rows.Count > 0)
            {
                if (dt.Rows[0]["Status"].ToString() == "1")
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('" + dt.Rows[0]["Message"] + "');location.replace('Master_Bank.aspx')", true);
        ...
        protected void GvData_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                cls.ExecuteQuery("Exec ProcMaster_Bank 'IsActive','" + e.CommandArgument + "'");
                FillData();
            }
