...
{
    public partial class Manage_PopularCategory : System.Web.UI.Page
    {
        ...
        {
            DataTable dtData = cls.selectDataTable("ProcManage_Popular 'GetAll'");
            GvData.DataSource = dtData;
            GvData.DataBind();
        }
        private void GetData(int id)
        {
            DataTable dtresult = cls.selectDataTable("ProcManage_Popular 'GetAll','" + id + "'");
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (filecategory.HasFile)
            {
                ...
                {
                    max = cls.ExecuteIntScalar("select Isnull(Max(ID),0)+1 from tblManage_Popular");
                }
                imageUpload = UploadImage(filecategory, max.ToString() + "_Icon");
            ...
            DataTable dt = cls.selectDataTable("Exec ProcManage_Popular 'insert','" + id + "','" + txtName.Text.Replace("'", "").Trim() + "','" + imageUpload.ImgName + "',N'" + txtDescription.Text.Replace("'", "").Trim() + "'");
            if (dt.Rows.Count > 0)
            {
                if (dt.Rows[0]["Status"].ToString() == "1")
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('" + dt.Rows[0]["Message"] + "');location.replace('Manage_PopularCategory.aspx')", true);
        ...
        private ImageUploadStatus UploadImage(FileUpload file, string Number)
        {
            ...
            if (file.HasFile == true)
            {
                if (file.PostedFile.FileName != "")
                {
                    ...
                    {
                        string opath = Server.MapPath("../Upload/Icon/");
        ...
        {
            if (e.CommandName == "IsChange")
            {
                GetData(Convert.ToInt32(e.CommandArgument));
            }
            else if (e.CommandName == "IsDelete")
            {
                cls.ExecuteQuery("Exec ProcManage_Popular 'IsDelete','"+e.CommandArgument+"'");
                FillData();
            }
        }
